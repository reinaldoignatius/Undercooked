...
class Agent():
    ...
    def __a_star(self, current_map, origin, destination):
        ...
        while open_set:
            ...
            if current[0] - 1 == destination[0] and current[1] - 1 == destination[1] or \
                ...
                path['should_dash'] = True if abs(x_direction) > 1 or \
                    abs(y_direction) > 1 else False
            ...
            for neighbor in neighbors:
                if not neighbor in closed_set:
                    ...
                    f_score[neighbor] = g_score[neighbor] + \
                        ...
                        abs(destination[1] - neighbor[1])) / game_constants.DASH_DISTANCE
    ...
    def translate_to_state(self, game_info, blackboard_recent_writings):
        ...
        for current_order in game_info['current_orders']:
            ...
            Encode order name:
                ...
                column 2: is b
            """
            if current_order.name == game_constants.ORDER_A_NAME:
                state.append(1)
                state.append(0)
            elif current_order.name == game_constants.ORDER_B_NAME:
                state.append(0)
                state.append(1)
        ...
        ingredients = copy.copy(game_info['ingredients'])
        own_chef = game_info['chefs'][int(self.name[-1:]) - 1]
        ...
        for plate in game_info['plates']:
            ...
            if plate.x == own_chef.x and plate.y == own_chef.y:
                ...
                state.append(0)
                state.append(0)
            elif plate.x == constants.PASSING_TABLE_X:
                state.append(0)
                state.append(1)
                state.append(0)
                state.append(0)
        ...
        for __, blackboard_recent_writing in blackboard_recent_writings.items():
            for idx in range(len(constants.ALL_ACTION_CHOICES)):
                state.append(1 if idx == blackboard_recent_writing else 0)
    ...
    def replay(self, batch_size):
        ...
        if self.epsilon > constants.MINIMUM_EPSILON:
